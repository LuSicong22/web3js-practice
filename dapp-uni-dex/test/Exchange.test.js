...
contract("Exchange", ([deployer, feeAccount, user1, user2]) => {
  ...
  describe("withdrawing Ether", () => {
    ...
    describe("success", () => {
      it("tracks the Ether withdraw", async () => {
        ...
        balance
          .toString()
          .should.equal((depositAmount - withdrawAmount).toString());
      });
      ...
      it("emits a Withdraw event", async () => {
        ...
        event.balance
          .toString()
          .should.equal(balance.toString(), "balance is correct");
      });
    });
  ...
  describe("depositing tokens", () => {
    ...
    describe("failure", () => {
      ...
      it("fails when no tokens are approved", async () => {
        await exchange
          .depositToken(token.address, amount, { from: user2 })
          .should.be.rejectedWith(EVM_REVERT);
      });
    });
  });
  ...
  describe("making orders", () => {
    ...
    it("tracks new order", async () => {
      ...
      order.amountGet
        .toString()
        .should.equal(amountGet.toString(), "amount get is correct");
      order.tokenGive.should.equal(tokenGive, "token give is correct");
      order.amountGive
        .toString()
        .should.equal(amountGive.toString(), "amount give is correct");
      order.timestamp
        .toString()
    ...
    it("emits Order event", async () => {
      ...
      event.id
        .toString()
        .should.equal(orderCount.toString(), "order id is correct");
      event.user
        .toString()
        .should.equal(user1.toString(), "user address is correct");
      event.tokenGet
        .toString()
        .should.equal(tokenGet.toString(), "token get address is correct");
      event.amountGet
        .toString()
        .should.equal(amountGet.toString(), "amount get is correct");
      event.tokenGive
        .toString()
        .should.equal(tokenGive.toString(), "token give address is correct");
      event.amountGive
        .toString()
        .should.equal(amountGive.toString(), "amount give is correct");
      event.timestamp
  ...
  describe("order actions", () => {
    ...
    beforeEach(async () => {
      ...
      // deployer gives tokens to user 2
      await token.transfer(user2, ether(100), { from: deployer });
      // user 2 deposits tokens only
      await token.approve(exchange.address, tokens(2), { from: user2 });
      await exchange.depositToken(token.address, tokens(2), { from: user2 });
      // user 1 makes order to buy tokens with ether
        ...
        it("emits Cancel event", async () => {
          ...
          event.tokenGet
            ...
            .should.equal(
              ...
              "token get address is correct"
            );
          event.amountGet
            .toString()
            .should.equal(tokens(1).toString(), "amount get is correct");
          event.tokenGive
            .toString()
            .should.equal(
              ETHER_ADDRESS.toString(),
              "token give address is correct"
            );
          event.amountGive
            .toString()
            .should.equal(ether(1).toString(), "amount give is correct");
          event.timestamp
            .toString()
            .length.should.be.at.least(1, "timestamp is present");
        });
    ...
    describe("filling order", () => {
      describe("success", () => {
        ...
        it("excutes trade and charges fess", async () => {
          let balance;
          // user 1 received tokens
          balance = await exchange.tokens(token.address, user1);
          balance
            .toString()
            .should.equal(tokens(1).toString(), "user1 received tokens");
          // user 2 received ether
          balance = await exchange.tokens(ETHER_ADDRESS, user2);
          balance
            .toString()
            .should.equal(ether(1).toString(), "user 2 received ether");
          // user 1 deduced ether
          balance = await exchange.tokens(ETHER_ADDRESS, user1);
          balance.toString().should.equal("0", "user 1 deducted ether");
        ...
        it("emits Trade event", async () => {
          ...
          const event = log.args;
          event.id.toString().should.equal("1", "trade id is correct");
          event.user
            .toString()
            .should.equal(user1.toString(), "user address is correct");
          event.tokenGet
          ...
          event.userFill
            .toString()
            .should.equal(user2.toString(), "user fill address is correct");
          event.timestamp
            .toString()
            .length.should.be.at.least(1, "timestamp is present");
        });
      ...
      describe("failure", () => {
        it("rejects order id less than 1", async () => {
          await exchange
            .fillOrder("0", { from: user2 })
            .should.be.rejectedWith(EVM_REVERT);
        });

        it("rejects order id greater than orderCount", async () => {
          await exchange
            .fillOrder("100", { from: user2 })
            .should.be.rejectedWith(EVM_REVERT);
        });
